...
var goBaseAddress = "https://go.dev/dl/"

func Download(version string, arch string, root string) bool {
	url := "https://go.dev/dl/go" + version + ".windows-" + arch + ".msi"
	filedir := root + "\\" + "go" + version + ".windows-" + arch + ".msi"
	...
	if response.Status[0:3] != "200" {
		...
		fmt.Println(response.Status[0:3])
	...
	fmt.Println("Opening Graphical installer")
	defer exec.Command("cd", root)
	defer exec.Command("go" + version + ".windows-" + arch + ".msi")
...
func DownloadDetached(version string, arch string, root string) bool {
